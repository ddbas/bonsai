...
- [x] 1.4 Add one shared classification function,
...
- [x] 1.5 Add
      ...
      through `classify()`.
- [x] 1.6 Add a `slot_status(path: &Path) -> Result<SlotStatusReport>` (naming
...
- [x] 1.7 Switch `find_available_slot` from `count_open_processes(&path)? > 0`
...
- [x] 1.8 Add unit tests for the new lock-reason parsing (with and without a
...
- [x] 2.1 Change the `Commands::List` handler in `src/main.rs` to call the new
...
- [x] 2.2 Remove the stats-column rendering logic from the `List` handler (the
...
- [x] 2.3 Remove `format_stats` and its unit tests in `src/main.rs` once
      confirmed unused (`bs status`'s renderer uses its own itemized format, not
      the compact stats string).
- [x] 2.4 Update the `List` subcommand's clap doc comment to describe the
...
- [x] 4.1 Update `tests/worktree_list.rs`: remove assertions on the stats column
...
- [x] 4.3 Add a unit test asserting `bs list`'s `classify_slot_status` and
...
- [x] 4.4 Run the full test suite and fix any regressions.

## 5. Performance benchmarking

- [x] 5.1 Add a Criterion dev-dependency and `benches/bs_ls.rs` benchmarking the
...
- [x] 5.4 Once `bs list` switches to `classify_slot_status` (tasks 2.x), re-run
