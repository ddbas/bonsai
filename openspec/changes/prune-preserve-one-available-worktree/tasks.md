...
- [x] 1.1 Update `prune_available_slots` (or add a new helper) so it returns the
...
- [x] 1.2 Add a helper (e.g. `select_prune_candidates`) that, given the
...
- [x] 1.3 Extend `PruneOutcome` with a `preserved: Option<PrunedSlot>` field
...
- [x] 2.1 In `prune_pool`, after computing `preserved`/`to_delete`: if
...
- [x] 2.2 On detach success, populate `outcome.preserved` with the
      `PrunedSlot { path, branch }` capturing the _pre-detach_ branch name (so
      reporting can say what was detached).
- [x] 2.3 On detach failure, populate the new failure field with the path and
...
- [x] 2.4 Continue deleting every slot in `to_delete` exactly as today
      (`delete_slot_dir`, collecting successes into `outcome.pruned` and
      failures into `outcome.failures`).
- [x] 2.5 Ensure `prune_pool` still calls `git_worktree_prune()` exactly once at
...
- [x] 3.1 Update the `bs prune` output to print a line for the preserved slot
...
- [x] 3.2 Update the "nothing to prune" branch's condition to also account for
      ...
      what was kept.
- [x] 3.3 Surface a preserve-detach failure the same way deletion failures are
      surfaced today (`eprintln!` + inclusion in the non-zero-exit failure
      count).
- [x] 3.4 Update the `bs prune` subcommand's doc comment/help text to mention
      that one available slot is always preserved (and detached if needed).

## 4. Spec/regression tests

- [x] 4.1 Add/update unit tests in `src/worktree/mod.rs` for
      `select_prune_candidates` (or equivalent): zero, one, and multiple
      available slots.
- [x] 4.2 Add/update integration or unit-level tests for `prune_pool` covering:
...
- [x] 4.3 Update any existing tests/fixtures that assumed `bs prune` deletes
      _all_ available slots to instead assert one remains.
- [x] 4.4 Run the full test suite (`cargo test`) and lint/format checks used by
