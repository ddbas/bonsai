...
- [x] 1.1 Add a `prune_available_slots(pool_dir: &Path)` (or similarly named)
...
- [x] 2.1 Add a `Prune` variant to the `Commands` enum in `src/main.rs` with doc
      comments describing its behavior (mirroring the style of `List`/`Status`).
- [x] 2.2 Handle the empty/non-existent pool case identically in spirit to
      `Commands::List`'s handling (pool dir missing → friendly message, exit 0)
      before invoking the prune logic.
- [x] 2.3 Handle the "pool exists but nothing available" case with a distinct
      friendly message, still running `git worktree prune` before exiting 0.
- [x] 2.4 On success, print one line per pruned slot using the existing
      tilde-path + bold-branch-in-parentheses formatting (reuse
      `worktree::tilde_path` and the same formatting pattern as `bs list`).
- [x] 2.5 On partial failure (one or more slot deletions failed), print the
      failures clearly, still print the lines for slots that succeeded, and exit
      with a non-zero status; on full success exit 0.

## 3. Tests

- [x] 3.1 Add `tests/worktree_prune.rs` following the `GitEnv`-based conventions
      of `tests/worktree_list.rs`.
- [x] 3.2 Test: no pool directory yet → friendly message, exit 0, no git
      worktree prune errors.
- [x] 3.3 Test: pool with only locked/in-use slots → no directories deleted,
      friendly "nothing to prune" message, exit 0.
- [x] 3.4 Test: pool with a mix of locked, in-use, and available slots → only
...
- [x] 3.5 Test: pruned slot with a checked-out branch is reported with the
      branch name in parentheses; pruned detached-HEAD slot is reported with no
      branch suffix.
- [x] 3.6 Test (best-effort, platform permitting): a per-slot deletion failure
...
- [x] 4.1 Update `README.md` (and any command reference/help text) to document
      `bs prune`, consistent with how `list`/`status`/`lock` are already
      documented.
- [x] 4.2 Run `cargo fmt`, `cargo clippy`, and the full test suite; fix any
