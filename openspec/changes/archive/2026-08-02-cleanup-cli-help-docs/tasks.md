...
- [x] 1.1 Trim the top-level `about` string on `Cli` to a single concise
      sentence describing what `bs` does.
- [x] 1.2 Rewrite the `--log-level` doc comment to state accepted values
...
- [x] 2.1 Rewrite the `Get` variant's doc comment to a short summary (what it
...
- [x] 2.2 Rewrite the positional `branch` field's doc comment: what it does,
...
- [x] 2.3 Rewrite the `-b`/`new_branch` doc comment: what it does, default
      behavior (fails if branch exists), mutual exclusivity with `-B`.
- [x] 2.4 Rewrite the `-B`/`reset_branch` doc comment: what it does (create or
      reset), mutual exclusivity with `-b`.
- [x] 2.5 Rewrite the `--tmux-session` doc comment: what it does, default
...
- [x] 2.6 Rewrite the `--no-attach` doc comment: what it does, that it requires
      `--tmux-session` — drop the cleanup-mechanics aside.

## 3. Remaining subcommands (`list`, `current`, `help`, `lock`, `unlock`, `info`)

- [x] 3.1 Rewrite the `List` variant's doc comment to a short summary of what it
      prints, keeping the color-coding legend concise (one line).
- [x] 3.2 Rewrite the `Current` variant's doc comment to a short summary plus
      the exit-status fact (0 inside a managed slot, 1 otherwise), dropping
      elaboration.
- [x] 3.3 Rewrite the `Help` variant's doc comment to a one-line summary.
- [x] 3.4 Rewrite the `Lock` variant's and `reason`/`path` field doc comments to
      state what each does and its default, dropping mechanism detail about how
      git performs the lock.
- [x] 3.5 Rewrite the `Unlock` variant's and `path` field doc comment similarly.
- [x] 3.6 Rewrite the `Info` variant's doc comment to a short summary of what
      fields it prints and that it performs no filesystem writes, dropping the
      "useful as a first debugging step" framing.

## 4. Verification

- [x] 4.1 Run `cargo build` to confirm the crate still compiles after the doc
      comment edits.
- [x] 4.2 Manually run `bs --help` and `bs <command> --help` for every changed
...
- [x] 4.3 Run `cargo test` to confirm existing tests in `main.rs` are unaffected
