...
- [ ] 1.1 Archive the delta spec into `openspec/specs/worktree-list/spec.md` by
...
- [ ] 3.1 Remove the `- [ ] \`current\` subcommand: show the current
      worktree`checkbox from`TODO.md`

## 4. Tests

- [ ] 4.1 Add a unit test (or extend an existing integration test) covering the
      scenario where `bs list` is run from inside a managed slot — verify the
      output line contains `▶` and `(current)`
- [ ] 4.2 Add a test for the case where CWD is not inside any managed slot —
      verify no row contains `▶` or `(current)`
- [ ] 4.3 Add a test confirming that when `current_worktree()` returns `None`
      (e.g. pool absent), `bs list` renders normally without panicking or
      emitting stray indicators

## 5. Verification

- [ ] 5.1 Run `cargo test` and confirm all tests pass
- [ ] 5.2 Manually run `bs list` from inside a provisioned slot and confirm the
