...
- [ ] 1.1 In `run_lsof` (`src/worktree/mod.rs`), change the
      `Command::new(lsof_bin)` args from `["+d", ...]` to `["-w", "+d", ...]`.
- [ ] 1.2 In `run_lsof_count` (`src/worktree/mod.rs`), apply the same
      `["-w", "+d", ...]` args change.

## 2. Remove custom warning-filtering logic

- [ ] 2.1 Delete the `lsof_real_errors` function and its doc comment in
      `src/worktree/mod.rs`.
- [ ] 2.2 In `run_lsof`, use `stderr` directly (trimmed) instead of calling
      `lsof_real_errors`, updating the surrounding comments to reflect that `-w`
      prevents warnings rather than post-hoc filtering.
- [ ] 2.3 In `run_lsof_count`, apply the same change: use `stderr` directly
      instead of calling `lsof_real_errors`.
- [ ] 2.4 Update the doc comments on `has_open_files`/`count_open_processes`
...
- [ ] 3.1 Delete `lsof_real_errors_strips_docker_warnings`,
...
- [ ] 4.1 Run `cargo build` and `cargo test` to confirm the crate compiles and
      existing `has_open_files`/`count_open_processes` tests still pass.
- [ ] 4.2 Run `cargo clippy` to confirm no unused-code warnings remain after
