...
- [ ] 1.1 In `src/main.rs`'s `Commands::List` handler, define a constant (or
      ...
      added.
- [ ] 1.2 Compute each row's plain badge label (`"locked"` / `"in use"` /
      ...
      escape codes.
- [ ] 1.3 Update the row `println!` in `Commands::List` to print
...
- [ ] 2.1 Add a test in `tests/worktree_list.rs` covering a mixed pool with at
