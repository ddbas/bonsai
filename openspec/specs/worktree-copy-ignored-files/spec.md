...
Allow `bs get` to carry a configured set of gitignored files (e.g. `.env`, local
config files) from the origin worktree into a newly provisioned or reused pool
slot, so ephemeral task environments start with the local configuration they
need without manual copying.
