...
Provision a managed git worktree slot with a named branch already checked out,
so developers can begin branch-based work in a single `bs get -b|-B` invocation
without a separate `git checkout -b` step.
