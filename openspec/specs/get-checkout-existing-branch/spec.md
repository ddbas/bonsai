...
Provision a managed git worktree slot and check out an **existing** branch
into it in one step, so developers can grab a worktree for a branch that
already exists elsewhere in the repository (e.g. a teammate's feature branch)
without manually running `git checkout <branch>` inside the slot afterward.
