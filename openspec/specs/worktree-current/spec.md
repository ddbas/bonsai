...
Provide a `bs current` subcommand that identifies which bonsai-managed worktree
pool slot the user's shell is currently inside, printing its tilde-abbreviated
path and branch name so users can orient themselves without scanning `bs list`
output.
