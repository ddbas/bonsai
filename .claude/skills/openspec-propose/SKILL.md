...
description:
  Propose a new change with all artifacts generated in one step. Use when the
  user wants to quickly describe what they want to build and get a complete
...
3. **Get the artifact build order**
   ...
   - `applyRequires`: array of artifact IDs needed before implementation (e.g.,
     `["tasks"]`)
...
4. **Create artifacts in sequence until apply-ready**
   ...
   - Create the artifact file using `template` as the structure and write it to
     `resolvedOutputPath`
   - Apply `context` and `rules` as constraints - but do NOT copy them into the
     file
   ...
   - After creating each artifact, re-run
     `openspec status --change "<name>" --json`
   - Check if every artifact ID in `applyRequires` has `status: "done"` in the
     artifacts array
...
- Prompt: "Run `/opsx:apply` or ask me to implement to start working on the
  tasks."
