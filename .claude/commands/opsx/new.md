...
**Input**: The argument after `/opsx:new` is the change name (kebab-case), OR a
...
5. **Get instructions for the first artifact** The first artifact depends on the
   schema. Check the status output to find the first artifact with status
   "ready".
...
- Prompt: "Ready to create the first artifact? Run `/opsx:continue` or just
  describe what this change is about and I'll draft it."
...
- If a change with that name already exists, suggest using `/opsx:continue`
  instead
